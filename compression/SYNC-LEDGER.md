...
| 318 | (ours, OPEN) build-windows.sh: pins file, module-derived library builds, fail-fast fetch | ➖ upstream tooling (phase0 unaffected) | ➖ | ci/tools/windows-pins.sh = single source of Windows pins (loud guard, no defaults); WITH_COMPRESSION toggle ready for compression/ landing; enabler for #319. CodeRabbit round: verify() cache poisoning was real (6ce1a7c: download to .part, rename after curl exits clean); enable-static-vcruntime is a no-op under no-shared (static libs already /MT /Zl, same CRT as nginx's -MT) — recorded as a comment |
| 319 | (ours, OPEN, stacked on #318) bump-versions.sh learns the harness and Windows pins; windows-build.yml reads windows-pins.sh | ➖ upstream tooling (phase0 unaffected) | ➖ (fork has no Windows pins, resolves mainline at run time) | eilandert asked for it on #313; harness pin in both workflows must agree, release-API pins move forward only, nginx digests through the PGP path; test_bump_versions.sh 5→10 cases and now run by build-test.yml; live: re-derives the three hand-recorded digests. Supersedes #313's workflow half once #318+#319 land |
