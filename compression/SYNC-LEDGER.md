...
| 226 | single-pass Accept-Encoding parameter scan | ✅ 1472e9e | YES (fork, hardening) | our parser was written to #142's boundary rules; whether its parameter scan is one pass is unverified — check alongside #315; inspected: both parsers took the cursor by value and rescanned; the single-pass contract ported with the DQUOTE-in-name carve-out; fork proven under its fuzz oracle (3.28M execs, no divergence) |
| 227 | 32-bit off_t hardening of the streaming max_length cap | ✅ 181fc31 | ➖ N/A (brotli bounds at off_t width before its only cast) | ours compares in off_t (module.c:2764) but the directive slot is ssize_t — same class; verify on an ILP32 build; inspected: the streaming cap had the exact `(off_t)` truncation shape; predicate + verbatim-extraction fixture, native and -m32 (CI requires the -m32 pass) |
