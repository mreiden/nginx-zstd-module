...
| 195 | read dictionary files to completion | ✅ 6943086 (+ unit 43d7bf2) | ✅ already present | dict.c cites #195; fork loader inspected: it has had the EINTR-retry, short-count-resuming read loop and the O_NONBLOCK clear since its #165/#199 lineage, so nothing to port |
...
| 226 | single-pass Accept-Encoding parameter scan | ✅ 1472e9e | YES ce25952 | ported: eval_qvalue took its cursor by value and the walker rescanned every parameterised element; it now advances the caller's cursor (next top-level `,` or end on success, the offending byte on -1). A DQUOTE inside a parameter NAME is the one input where the resume position differs, so the cursor stays on the `;` there and the historical result holds. Fork twin proven under its differential fuzz oracle (3.28M execs, no divergence) |
| 227 | 32-bit off_t hardening of the streaming max_length cap | ✅ 181fc31 | ➖ N/A (brotli bounds at off_t width before its only cast) | ported: the cap check was `(off_t) bytes_in > (off_t) max_length`, truncating the uint64_t accumulator on a 4-byte off_t; it is now `ngx_http_compression_max_length_exceeded(uint64_t, ssize_t)`, an unsigned compare behind the unset/negative guard. tools/test_max_length_cap_unit.{c,sh} extracts it verbatim and runs it native and plain -m32 (CI installs gcc-multilib and sets REQUIRE_M32) |
...
| 314 | static: fail closed on duplicate Available-Dictionary; stale-errno probe log | ◑ (a) N/A by design, (b) ✅ efcfcf3 | ➖ | (a) no static→filter handoff here, and match_dict already counts and refuses duplicates; (b) ported: a short read that SUCCEEDED (n >= 0) is logged once at [error] with errno 0 and memoized as malformed; CRIT with ngx_errno is reserved for n < 0 (it was CRIT with a stale errno for both); t/03 TEST 22d |
| 315 | chain walker parses each field line once (`_ex` out-params) | ✅ ec3a30b | YES 542d9e5 | ported: fold_line_weight calls the `_ex` parser once per field line and reads explicit_q/star_q from its out-params (it called the single-line parser twice per line). phase0 dropped the non-ex wrapper (-Werror unused); the fork keeps it as the fuzz oracle's entry point and its extraction regex knows `_ex` |
| 316 | strict dict walk vets ancestor directories | ✅ 67770c1 (security) | YES b2c9d54 | ported: check_strict_dir vets the root and every intermediate directory fd for group/other write, not the leaf only (both walks checked the leaf only before). Fixtures moved from /tmp to $HOME with a vetted ancestor chain and skip_eval when unclean: t/01 TESTs 27a-27c, fork t/01-dcb TESTs 26a-26c |
...
| 321 | A33 micro sweep P7–P11 | ◑ P7 ✅ efcfcf3, P8 ✅ 181fc31, P9 ✅ via sync b6f1386, P10/P11 N/A | ➖ | P7 bounds the bad-verdict scan by populated slots (our bad[64] memo can take it); P8 lowcase_key memcmp for Available-Dictionary (our walk uses strncasecmp, module.c:500); P9 changes the shared cache-control header's directive_end signature — we call only the top-level no_transform() and the seam test counts the same three definitions, so his sync is safe; P10 Vary token walk N/A (Vary by construction); P11 loc-conf threading N/A (one fetch, module.c:1876) |
| 322 | binary-search the dict negotiation lookup above 16 entries | ✅ 9789f63 | 🔎 | ported: match_dict goes through dict_lookup, a binary search over the digest-sorted pointer array above NGX_HTTP_COMPRESSION_DICT_BSEARCH_THRESHOLD (16) and the linear memcmp scan at or below it (it was the linear scan at every count). The array is sorted on OWN and on INHERIT, since http-level arrays only ever reach a location as `prev`. t/02 TESTs 30-34: mid-list, first-declared, miss, and both sides of the threshold; a sort-removal mutant fails TESTs 30/31 |
...
### Batch: #327–#335 (2026-09-14 → 2026-09-15) — all ours; synced by merge 471b062

| # | subject | disposition | brotli? | notes |
|---|---------|-------------|---------|-------|
| 327 | (ours) one nginx release resolver for the 8 workflow resolve steps, ci-build.sh and the bump | ➖ upstream tooling (phase0 unaffected: compression.yml pins NGINX_VERSION statically) | ➖ (the fork's ci-build.sh still scrapes nginx.org — vendor the resolver at handover) | merged 428c642 after his second exact-head round (per-job bootstrap check with a helgrind-strip control; no-odd feed control; provenance comments; nginx_lines() returns 1 where the read fails) |
| 328 | (ours) dict-file header, half 1: `ngx_http_zstd_dict_file_read()` + `ngx_http_zstd_hex_nibble()` | ✅ ADOPTED (this sync) | YES at handover (the fork's read loop is the same contract; rename one call) | dict.c now includes ../src/ngx_http_zstd_dict_file.h; its own read loop deleted (the call site already tested `n != len`), hex_decode decodes through the shared nibble; tools/test_read_dict_file_unit.{c,sh} + the compression.yml step deleted (the parent's fixture covers the header) |
| 329 | (ours) dict-file header, half 2: the strict walk returning a code | ✅ ADOPTED (this sync) | YES at handover (fork keeps its `brotli_dcb_dict_strict_path` message switch) | superseded upstream by #330 (closed, its commits merged there); check_strict_dir + open_dict_strict deleted from dict.c, replaced by `ngx_http_compression_log_dict_walk()` (the ten messages, verbatim, naming compression_dict_strict_path) and a shell over `ngx_http_zstd_dict_file_open_strict()` |
| 330 | (ours) component scanner split out of the walk; trailing separator names a directory; 6 config blocks | ✅ ADOPTED (this sync), behaviour change: `compression_dict_file /x/w.dict/;` under strict on is now REFUSED | YES at handover | t/01-dict-store.t TEST 27d pins the refusal on a regular-file leaf (twin of parent TEST 27); README strict paragraph gains the sentence; parent's fixture is the negative control (fails on the pre-fix walk on exactly the four separator assertions) |
...
| # | subject | disposition | brotli? | notes |
|---|---------|-------------|---------|-------|
| 336 | config-load warning when dcz dictionaries meet an expensive profile (`zstd_comp_level` ≥ 9 or `zstd_long on`; names the count and the largest dictionary) | 🔜 twin worth doing, zstd arm only: the zstd backend attaches through `ZSTD_CCtx_refPrefix()` per request exactly as the parent does, so `compression_level zstd ≥ 9` with `compression_dict_file` lines has the same measured profile (parent: 1 MB dict 0.75 ms at level 3 vs 4.5 ms at level 9). No long-mode arm (no `compression_long`). The brotli backend prepares its dictionary per request too (`BrotliEncoderPrepareDictionary`, quality-baked) — a second arm is plausible but UNMEASURED; measure before adding it | ➖ (the fork has no dictionary attach yet) | merged 58bda87; parent TESTs 32-35b in ci/t/02-conf-warn.t are the template (level arm, default-level control, count + largest-size rendering, level-without-dicts control) |
