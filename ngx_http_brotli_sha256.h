/*
 * Minimal SHA-256 (FIPS 180-4) for hashing dcb dictionaries at config
 * load.
 *
 ...
 * and the 32-byte hash embedded in the dcb frame header), but nginx core
 ...
 * hard dependency.
 *
 * static inline (matching ngx_http_brotli_common.h's pattern): only the
 * filter TU uses it today; inline definitions are exempt from
 * -Werror=unused-function if the static module ever includes this.
 */

#ifndef NGX_HTTP_BROTLI_SHA256_H
#define NGX_HTTP_BROTLI_SHA256_H
...
#define NGX_HTTP_BROTLI_SHA256_DIGEST_LEN  32
...
typedef struct {
    ...
    size_t    block_len;
} ngx_http_brotli_sha256_t;


static const uint32_t  ngx_http_brotli_sha256_k[64] = {
...
static void
ngx_http_brotli_sha256_compress(ngx_http_brotli_sha256_t *c, const u_char *p)
{
    uint32_t    w[64], a, b, d, e, f, g, h, s0, s1, t1, cc, ch, maj;
    ngx_uint_t  i;

#define ngx_http_brotli_rotr(x, n)  (((x) >> (n)) | ((x) << (32 - (n))))
    ...
    for (i = 16; i < 64; i++) {
        s0 = ngx_http_brotli_rotr(w[i - 15], 7) ^ ngx_http_brotli_rotr(w[i - 15], 18)
             ^ (w[i - 15] >> 3);
        s1 = ngx_http_brotli_rotr(w[i - 2], 17) ^ ngx_http_brotli_rotr(w[i - 2], 19)
             ^ (w[i - 2] >> 10);
    ...
    for (i = 0; i < 64; i++) {
        s1 = ngx_http_brotli_rotr(e, 6) ^ ngx_http_brotli_rotr(e, 11)
             ^ ngx_http_brotli_rotr(e, 25);
        ch = (e & f) ^ (~e & g);
        t1 = h + s1 + ch + ngx_http_brotli_sha256_k[i] + w[i];
        s0 = ngx_http_brotli_rotr(a, 2) ^ ngx_http_brotli_rotr(a, 13)
             ^ ngx_http_brotli_rotr(a, 22);
        maj = (a & b) ^ (a & cc) ^ (b & cc);
...
#undef ngx_http_brotli_rotr
}


static void
ngx_http_brotli_sha256_init(ngx_http_brotli_sha256_t *c)
{
...
static void
ngx_http_brotli_sha256_update(ngx_http_brotli_sha256_t *c, const u_char *data,
    size_t len)
{
    size_t  n;
    ...
    if (c->block_len) {
        ...
        ngx_http_brotli_sha256_compress(c, c->block);
        c->block_len = 0;
    }

    while (len >= 64) {
        ngx_http_brotli_sha256_compress(c, data);
        data += 64;
        len -= 64;
    }
...
static void
ngx_http_brotli_sha256_final(ngx_http_brotli_sha256_t *c,
    ...
    u_char digest[NGX_HTTP_BROTLI_SHA256_DIGEST_LEN])
{
    ...
    ngx_http_brotli_sha256_update(&c, data, len);
    ngx_http_brotli_sha256_final(&c, digest);
}


#endif /* NGX_HTTP_BROTLI_SHA256_H */
